...
def make_dataset(cfg: TrainPipelineConfig, dataset_cfg: DatasetConfig | None = None) -> LeRobotDataset | MultiLeRobotDataset:
    """Handles the logic of setting up delta timestamps and image transforms before creating a dataset.

    Args:
        cfg (TrainPipelineConfig): A TrainPipelineConfig config which contains a DatasetConfig and a PreTrainedConfig.
        dataset_cfg (DatasetConfig, optional): A DatasetConfig to use instead of cfg.dataset.
    ...
    if isinstance(dataset_cfg.repo_id, str):
        ...
        if not dataset_cfg.streaming:
            dataset = LeRobotDataset(
                ...
                tolerance_s=1.0   # HACK: for lbm-eval-train dataset, the tolerance is set to 1 seconds to avoid frame out of range error
            )
        else:
