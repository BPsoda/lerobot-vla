...
try:
    ...
    from robot_gym.multiarm_spaces_conversions import multiarm_action_to_dp_vector, dp_vector_to_multiarm_action
...
def rotation_matrix_to_rot6d(rot_matrix: np.ndarray) -> np.ndarray:
    ...
    rot6d = rot_matrix[...,:2,:].reshape(batch_shape + (6,))
...
def convert_multiarm_observation_to_lerobot(
    ...
    if hasattr(observation, "robot") and hasattr(observation.robot, "actual"):
        ...
        state_vec = multiarm_action_to_dp_vector(actual)
        ...
        if hasattr(actual, "poses") and actual.poses:
            ...
            if state_components:
                ...
                assert np.allclose(state_tensor.numpy(), state_vec), f"State tensor and state vector mismatch: {state_tensor.numpy()} != {state_vec}"
...
class ReplayPolicy(Policy):
    ...
    def step(self, observation: MultiarmObservation) -> PosesAndGrippers:
        ...
        logger.info(f"Difference between expected and actual state: {obs_state - expected_state}")
